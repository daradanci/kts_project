...
class GetGameInfoView(AuthRequiredMixin,View):
    @docs(tags=["web"], summary="Game info", description="Get last game info with chat_id")
    @response_schema(OkResponseSchema)
    async def get(self):
        game_info=await self.store.game.get_last_game(self.request.rel_url.query['chat_id'])
        print(game_info)
...
class GetChatInfoView(AuthRequiredMixin,View):
    @docs(tags=["web"], summary="Get chat info", description="Get chat info using chat_id and tg_id")
    @response_schema(OkResponseSchema)
    async def get(self):
        chat_info=await self.store.game.get_chat_info(chat_id=self.request.rel_url.query['chat_id'],
...
class StartGameView(AuthRequiredMixin,View):
    @docs(tags=["web"], summary="Start game", description="Start new game with chat_id")
    @request_schema(GameSchema)
    @response_schema(OkResponseSchema)
    async def post(self):
        new_game=await self.store.game.start_game(chat_id=self.data['chat_id'], players=self.data['players'])

        return json_response(data={
            'id':new_game.id,
            'chat_id':new_game.chat_id,
            'created_at':str(new_game.created_at),
            'players': [{'tg_id':player.tg_id,
                         'name':player.name,
                         'last_name':player.last_name,
                         'score':[score.points for score in player.score]
